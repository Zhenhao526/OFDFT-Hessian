...
def prepare_md(args: argparse.Namespace) -> None:
    ...
    for volume_per_atom in args.volumes_per_atom:
        ...
        write_job(
            ...
            element,
            config,
            job_type="mpn_nvt_zero_pressure_volume_scan",
...
def analyze_md(args: argparse.Namespace) -> None:
    ...
    for point in manifest["points"]:
        ...
        phase_result = analyze_phase(run, manifest["phase"])
        ...
        rows.append(
            {
                **point,
                ...
                "temperature_last_half_K": series_stats(temperatures),
                "nearest_neighbor_A": phase_result.get("trajectory", {}).get("nearest_neighbor_A"),
            }
        )
    valid = [row for row in rows if row["phase_status"] == f"{manifest['phase']}_verified" and row["pressure_last_half_kbar"]]
    result = {**manifest, "rows": rows, "valid_phase_points": len(valid), "linear_fit": linear_fit_zero(valid)}
