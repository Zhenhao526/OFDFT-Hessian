...
def component_losses(
    ...
    matrix_free_protocol = protocol["matrix_free_geometry_response"]
    matrix_free_settings = matrix_free.MatrixFreeSettings(
        tolerance=float(matrix_free_protocol["tolerance"]),
        max_iterations=int(matrix_free_protocol["max_iterations"]),
        preconditioner=str(matrix_free_protocol["preconditioner"]),
        preconditioner_block_size=int(
            matrix_free_protocol["preconditioner_block_size"]
        ),
    )
    with matrix_free.matrix_free_parameter_context(
        parameters, matrix_free_settings
    ):
...
def main() -> None:
    ...
    parser.add_argument("--probe-molecule-id")
    ...
    matrix_free_protocol = protocol["matrix_free_geometry_response"]
    overlay_path = Path(matrix_free_protocol["overlay_path"])
    if overlay_path.resolve() != Path(matrix_free.__file__).resolve():
        raise RuntimeError("matrix-free overlay path mismatch")
    if sha256(overlay_path) != matrix_free_protocol["overlay_sha256"]:
        raise RuntimeError("matrix-free overlay SHA256 mismatch")
    synthetic_report_path = Path(
        matrix_free_protocol["synthetic_direct_comparison_report"]
    )
    if (
        sha256(synthetic_report_path)
        != matrix_free_protocol["synthetic_direct_comparison_report_sha256"]
    ):
        raise RuntimeError("matrix-free synthetic audit SHA256 mismatch")
    synthetic_report = json.loads(synthetic_report_path.read_text())
    if synthetic_report.get("status") != "pass":
        raise RuntimeError("matrix-free synthetic direct comparison did not pass")
    if (
        matrix_free_protocol["solver"] != "tangent_minres"
        or matrix_free_protocol["backward"]
        != "implicit_adjoint_second_matrix_free_solve"
        or bool(matrix_free_protocol["backward_iterative_unroll"])
        or bool(matrix_free_protocol["detached_parameter_vjp"])
        or matrix_free_protocol["preconditioner"]
        != "streaming_exact_abs_block_diagonal"
        or int(matrix_free_protocol["preconditioner_block_size"]) != 512
        or bool(matrix_free_protocol["preconditioner_global_matrix_assembly"])
        or not bool(matrix_free_protocol["preconditioner_local_block_assembly"])
        or bool(matrix_free_protocol["preconditioner_create_graph"])
        or not bool(matrix_free_protocol["explicit_residual_replacement"])
        or int(matrix_free_protocol["max_total_iterations"]) != 10000
    ):
        raise RuntimeError("matrix-free protocol semantic binding mismatch")
    ...
    if args.probe_molecule_id is not None:
        if args.mode != "probe" or enabled or args.reference_global_batch:
            raise ValueError(
                "probe-molecule-id requires a single-process probe without "
                "reference-global-batch"
            )
        if args.probe_molecule_id not in train_ids:
            raise ValueError("probe-molecule-id must belong to frozen Train80")
        local_ids = [args.probe_molecule_id]
    elif args.reference_global_batch:
    ...
    if rank == 0:
        atomic_json(args.output_dir / "run_registration.json", {
            ...
            "matrix_free_geometry_response": protocol[
                "matrix_free_geometry_response"
            ],
            "matrix_free_overlay_path": str(Path(matrix_free.__file__).resolve()),
            "matrix_free_overlay_sha256": sha256(
                Path(matrix_free.__file__).resolve()
            ),
            "probe_molecule_id": args.probe_molecule_id,
            "test_accessed": False,
        })
    ...
    for update in range(args.steps):
        ...
        for molecule in active:
            ...
            diagnostics["matrix_free_response"] = (
                matrix_free.consume_matrix_free_diagnostics()
            )
