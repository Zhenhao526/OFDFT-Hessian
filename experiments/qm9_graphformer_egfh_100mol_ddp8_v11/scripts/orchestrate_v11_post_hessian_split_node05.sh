...
data_orchestrator="$root/runs/qm9_graphformer_egfh_100mol_ddp8_v11/data_orchestrator"
run_dir="$root/runs/qm9_graphformer_egfh_100mol_ddp8_v11/post_hessian_split"
...
expected_data_orchestrator_pid=3093787
...
printf 'running stage=wait_hessian at=%s\n' "$(date --iso-8601=seconds)" > "$status_file"
date --iso-8601=seconds > "$run_dir/started_at.txt"

while true; do
  upstream_status=$(cat "$data_orchestrator/status.txt" 2>/dev/null || true)
  printf '%s %s\n' "$(date --iso-8601=seconds)" "$upstream_status" >> "$run_dir/wait.log"
  if [[ "$upstream_status" == complete* ]]; then
    break
  fi
  if [[ "$upstream_status" == failed* ]]; then
    echo "ERROR: upstream data orchestrator failed: $upstream_status" >&2
    exit 4
  fi
  if ! kill -0 "$expected_data_orchestrator_pid" 2>/dev/null; then
    echo "ERROR: upstream data orchestrator disappeared without completing" >&2
    exit 5
  fi
  sleep 30
done
