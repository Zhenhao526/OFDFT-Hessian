...
migrated_checkpoint=${family}/node05_joint_v10_sparse_eval100_ckpt50_migration_20260811a/step_0000080_relaxed_tol1e9.ckpt
schedule_migrated_checkpoint=${family}/node05_joint_v10_sparse_eval100_ckpt50_migration_20260811a/step_0000080.ckpt
rebind_script=${repo}/scripts/rebind_qm9_v10_schedule_calibration.py
calibration_dir=${family}/node05_joint_v10_sparse_eval100_ckpt50_calibration_20260811a
calibration=${calibration_dir}/calibration.json
output=${family}/node05_joint_v10_sparse_eval100_ckpt50_resume_s80to300_20260811a
monitor_dir=${family}/step300_sparse100_ckpt50_monitor_20260811a
...
main() {
  ...
  if [[ ! -s "${schedule_migrated_checkpoint}" ]]; then
    env PYTHONPATH="${repo}${PYTHONPATH:+:${PYTHONPATH}}" \
      "${python_bin}" "${repo}/scripts/migrate_qm9_v10_schedule_checkpoint.py" \
      --source "${source_checkpoint}" \
      --destination "${schedule_migrated_checkpoint}" \
      --protocol "${protocol}" \
      --expected-step 80
  fi
...
PY
  ...
  if [[ ! -s "${migrated_checkpoint}" ]]; then
    env PYTHONPATH="${repo}${PYTHONPATH:+:${PYTHONPATH}}" \
      "${python_bin}" "${rebind_script}" \
      --source "${schedule_migrated_checkpoint}" \
      --destination "${migrated_checkpoint}" \
      --calibration "${calibration}" \
      --expected-step 80 \
      --lambda-h-abs-tol 1.0e-9
  fi
  ...
  write_status training 80 "joint E/G/F/H training to step 300; checkpoint every 50 steps and full Hessian every 100 steps"
  env \
    QM9_NODE02_REPO="${repo}" \
    QM9_CODE_ROOT="${repo}" \
    QM9_NODE_ACTIVATION_SCRIPT="${activation}" \
