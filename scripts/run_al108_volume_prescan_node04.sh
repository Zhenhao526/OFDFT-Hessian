#!/usr/bin/env bash
set -euo pipefail

root=/scratch/xzh/OFDFT-Hessian
base=$root/runs/al/free_energy/volume_scan/static_prescan
solid=$base/solid_T0850
liquid=$base/liquid_T0850
log=$base/volume_prescan_node04.log
mpirun=/scratch/xzh/conda/envs/abacus-gpu/bin/mpirun
abacus=$root/external/abacus-develop/build-gpu-gcc13-openmp/source/abacus_ml_gpu

mkdir -p "$base"
: > "$log"

points=(
    "$solid/vpa_17p000"
    "$solid/vpa_17p400"
    "$solid/vpa_17p800"
    "$solid/vpa_18p200"
    "$solid/vpa_18p600"
    "$liquid/vpa_17p500"
    "$liquid/vpa_18p200"
    "$liquid/vpa_18p900"
    "$liquid/vpa_19p600"
    "$liquid/vpa_20p300"
)

run_wave() {
    local first=$1
    local count=$2
    local pids=()
    for ((slot=0; slot<count; slot++)); do
        point=${points[$((first + slot))]}
        lo=$((slot * 12))
        hi=$((lo + 11))
        (
            cd "$point"
            exec env OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 \
                taskset -c "$lo-$hi" "$mpirun" -np 12 --bind-to none "$abacus" > run.stdout 2>&1
        ) &
        pids+=("$!")
        printf '%s started point=%s cpus=%s-%s pid=%s\n' "$(date -Iseconds)" "$point" "$lo" "$hi" "$!" >> "$log"
    done
    for pid in "${pids[@]}"; do
        wait "$pid"
    done
}

run_wave 0 6
run_wave 6 4

env PYTHONPATH="$root" python3 "$root/scripts/prepare_al108_volume_scan.py" analyze "$solid" >> "$log" 2>&1
env PYTHONPATH="$root" python3 "$root/scripts/prepare_al108_volume_scan.py" analyze "$liquid" >> "$log" 2>&1
printf '%s completed\n' "$(date -Iseconds)" >> "$log"
